        ...
        next: (data: any) => {
            ...
            if (data.notifications?.data) {
                data.notifications.data.forEach((note: any) => {
                    feedItems.push({
                        ...
                        content: note.text || note.message,
            ...
            if (data.subscriptions?.data) {
                data.subscriptions.data.forEach((sub: any) => {
                    feedItems.push({
                        ...
                        content: `Your ${sub.status} membership node is now live at ${sub.gym?.name}. Synchronizing training protocols.`,
            ...
            if (data.attendances?.data) {
                data.attendances.data.forEach((check: any) => {
                    feedItems.push({
                        type: 'TRAINING',
                        gymName: check.session?.course?.gym?.name || 'Regional Hub',
                        title: 'Biometric Check-in Confirmed',
                        content: `System synchronized at ${check.session?.course?.gym?.name} for training in ${check.session?.course?.name}.`,
